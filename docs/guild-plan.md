...
- 자발 탈퇴: 멤버 → guild_members 제거 + 7일 쿨다운. 마스터는 멤버가 있으면 거부, 혼자면 해체로 흐름 분기
- 마스터의 추방: 길드원 행에서 추방 → 대상자도 7일 쿨다운
- 마스터 양도: 멤버 중 1명을 새 마스터로 → role 스왑, 본인은 일반 멤버
- 강제 해체: 마스터 → 모든 멤버 affiliation 무소속 + 마스터에게만 7일 쿨다운 + `disbandedAt` 마킹
...
- 시스템 알림 우편 (해체/위임 통보)
- 탈퇴/추방 쿨다운 7일 enforce (서버는 이미 적용, 클라 UI는 데이터 기반 표시)
