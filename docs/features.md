...
- **오프라인 사냥** — 페이지를 떠난 동안에는 JS가 못 도므로, 복귀 시 `offlineSim.ts`로 결정적 시뮬레이션을 한 번에 돌려 결과를 적용. cap 30분 (`OFFLINE_SIM_MAX_MS`). 사망 시 break + 시작 마을 이동. 알림 한 줄로 요약. 트리거: mount + visibility hidden→visible. (`useOfflineSimulation`, 키 `last-active-tick.v1`)
