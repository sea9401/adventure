...
6. **M6 — 미스터리 + 히든 + 신규 NPC + 칭호** (§1.3 순례자, §6.2 사미승·문지기, §3.1 음유시인, §8 칭호, §11 히든 퀘스트 8종). 다음 콘텐츠 떡밥 심기.
   - ✅ *구현됨*: §1.3 순례자 `PilgrimDialogue` 3단계(`volcano_heart_defeated` → `skyreach_main_cleared` → `pilgrim_revealed`) + `STORY_QUESTS.pilgrim_beyond_north`. 신규 NPC 3명 — `windvale_bard`(음유시인 — 산정/화염 능선/천공 봉인 소문 + `hidden-lucky-collector` 떡밥), `skyreach_acolyte`(사미승 운하 — 첨탑 너머 떡밥), `skyreach_gatekeeper`(문지기 청람 — 화산 정기 의뢰 안내) — 전부 **초상화 미지정(placeholder, 추후 추가)**, 의뢰 없는 lore/quest NPC. 칭호 `caravan_warden`(노을 호위 의뢰 2종 완수, page.tsx 하드체크).
   - **미포함 (별도 작업 — 선행 필요)**: 히든 퀘스트 8종 — `hidden-mole-king`·`hidden-lucky-collector`(장비 보유 체크 헬퍼 필요) / `hidden-giants-origin`(adventureLog 처치수 체크 + PilgrimDialogue prop 확장) / `hidden-deepest-vein`(⚠ 신규 몬스터 "광물 골렘") / `hidden-volcano-relic`·`hidden-blacksmith-duel`(⚠ 신규 아이템, §13 미결정) / `hidden-pilgrim-trail`(지역 방문 추적 + 4단계) / `hidden-hooded-cipher`(StrangerDialogue·PilgrimDialogue relay → 순례자 상태 3). 칭호 `lucky_finder`·`cipher_bearer`(해당 히든 완료 시).
