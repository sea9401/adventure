...
3. **M3 — 사이드 의뢰 일괄** (§2 운향 / §3 다리 구간 / §4 봉황령 / §5 화산 지대). 길드판 반복 + NPC 전속 — 데이터만이라 빠름. 봉황 망토 확정 루트(§4) 포함.
   - **운향분(§2.1·§2.2·§3.2 운향 출처) + 게시판 supply-escort** ✅ *구현됨* — 도연 7의뢰(인트로 절벽 늑대 + stone-frogs·windspirits·jimmy-timber·cloud-raiders·bison-down·phoenix-hunt) / 산하 7의뢰(herbs·bones·nora-herbs·tough-hide·windstone·bison-hide·flame-scale, 전부 deliver) / 백운 사이드 `unhyang-baekun-pilgrim-escort` / 운향 게시판 9개(`unhyang-board-*`, supply-escort 포함). 기존 미노출이던 `unhyang-guide-cloud-raiders`·`unhyang-guide-phoenix-hunt`·`unhyang-herbalist-flame-scale` 도 다이얼로그에 연결. 새 헬퍼 `questLineDialogue.tsx`(다단계 의뢰 NPC 공용). 미포함: 바람골/봉황령/화산 출처분.
