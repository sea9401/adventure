...
- **자동 반복 토글** — "자동 전투" 버튼이 ON이면 한 판 끝난 직후 같은 지역에서
  다음 적과 자동으로 새 전투 시작. OFF면 한 판만 진행.
- 현재 지역의 적 풀(`region.enemies`)에서 랜덤 1마리와 1:1 전투.
- 선공은 플레이어 고정.
- 승리 시 EXP·골드, 패배 시 안전한 복귀(시작 마을로 회수, 자동 반복도 자동 OFF).
...
1. 전투 외 화면(현재 BattleView)에서 "자동 전투" 토글을 ON으로 두거나, OFF인 채
   "전투 시작" 1회 클릭.
...
6. **종료**:
   - 자동 전투 OFF → 결과 화면(BattleResult)에서 "확인" 클릭으로 외부 상태
     (EXP/HP/골드) 반영하고 전투 외 화면으로 복귀.
   - 자동 전투 ON & 승리 → 결과 잠깐(약 1.2s) 표시 후 자동 확인 → 외부 상태 반영
     → 같은 지역에서 새 적 랜덤 조우 → 2단계로 돌아감.
   - 자동 전투 ON & 패배 → 자동 토글 OFF로 강제 전환 + 시작 마을 복귀
     (자동 무한 사망 방지).
...
- **자동 전투 토글**: `battle-settings.v1`에 `{ auto: boolean }` 저장.
  새로고침 시 사용자 설정 유지. **패배 시 / region 이동 시** 강제 OFF.

## UI 분기

`BattleView`는 3-state 머신 + 자동 토글:
...
- 토글 ON 상태에서 "전투 시작"을 누르면 무한 루프가 시작됨.
- 토글 ON이지만 전투 외 상태일 때 "전투 시작"을 안 눌러도 자동 시작할지 여부는
  v1에선 **수동 시작**으로 둠 (안전). 토글은 "다음 전투 자동 진행" 의미.
- BattleScene 동안 토글 변경은 가능 — 다음 전투 진행 여부에만 영향.
...
8. **자동 토글 상태** — `battle-settings.v1` 영속, page.tsx 또는 BattleView에서 관리.
9. **`BattleView` 통합** — phase state로 3-state 분기. "전투 시작" 클릭 시 현재
   region에서 랜덤 enemy 선택 → BattleScene으로. 결과 확인 시 character 상태 업데이트.
   - 자동 ON & 승리 → 결과 1.2s 후 새 enemy로 다시 시작.
   - 자동 ON & 패배 → 토글 OFF로 강제 + `mapProgress.currentRegionId = "village"`로 강제.
...
- **턴 간격**: 500ms 고정.
- **반복**: 무한 가능, "자동 전투" 토글 ON일 때만 자동 재시작.
- **적 조우**: 현재 region 풀에서 랜덤 1마리.
- **선공**: 플레이어 고정.
- **패배 페널티**: 손실 없음 — 시작 마을 복귀 + HP 회복만.
- **자동 진행 시 결과 표시 시간**: 1.2s 후 자동 확인.
- **region 이동 시**: 자동 전투 토글 강제 OFF (`mapProgress.currentRegionId` 변경 감지 시
  `battle-settings.auto`를 false로 리셋).
