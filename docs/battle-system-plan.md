...
4. **자동 턴 루프** — `setTimeout`(또는 `setInterval`)으로 한 턴씩 진행:
   - 현재 턴 측이 공격 → damage 계산 → 상대 HP 감소 → 로그 추가 → 턴 교체.
   - 어느 한 쪽 HP ≤ 0 이면 종료.
   - 턴 간격: 기본 700ms (배속/스킵 옵션은 v2).
5. **플레이어 입력**은 한 가지뿐 — "도망" 버튼.
   - 누르면 즉시 평가: 고정 60% 확률 성공 → outcome=`fled` 종료, 실패 → 다음 적 턴 그대로 진행.
...
6. **`battle/useBattle.ts`** — start/flee 액션 + 턴 자동 진행(`setTimeout` 700ms
   재예약). 종료 시 `onEnd(outcome, rewards)` 콜백.
...
- BattleScene으로 진입 시 SubViewHeader 뒤로 가기 비활성/숨김 검토 (전투 도중
  뒤로 가면 전투 취소 vs 무시 — v1에선 비활성 추천).

## v2 이후

- **배속/스킵**: 1x / 2x / 즉시 종료 옵션 (관전 시간이 길어질수록 필요).
- **자동 도망 임계치**: HP 일정 % 이하 시 자동 도망 옵션.
