...
const STEPS: QuestLineStep[] = [
  {
    ...
    offerText:
      "별바다에 사람 발소리가 닿은 게 얼마만인지. 첨탑까지 올라온 자라면 이야기가 통하겠소. 회랑에 떠도는 시녀들의 잔영이 별빛 흐름을 흩어 놓고 있소 — 열둘만 가라앉혀 주오. 그러면 회랑검과 회랑 방패 벼림법을 자네에게 넘겨주겠소.",
  ...
  },
  {
    ...
    offerText:
      "회랑 깊은 곳에 별빛 망령들이 옛 흐름을 묶어두고 있소. 그들이 풀려야 회랑이 본래 모습으로 돌아오오. 열다섯만 풀어 주면 회랑창과 회랑 너클 벼림법도 함께 넘겨주겠소.",
    activeText: (have, need) =>
  ...
  // 별을 지키는 자 (Lv70, 별궤도 자율기 의뢰 완수 후)
  {
    ...
    offerText:
      "별을 지키는 자 협동 토벌에서 GOLD 이상 보상에 한 번이라도 닿아 보시오. 별빛이 자네를 한 번 깊이 알아본다면 — 그 기억은 평생 가오.",
    activeText: (have, need) =>
      `GOLD 등급은 단번에 닿지 않을 수도 있소. 호흡 가다듬고 다시 도전하시오. ${have}/${need}`,
    doneText:
  ...
  },
  {
    ...
    offerText:
      "별을 지키는 자에게 단 한 번의 일격으로 2,000 의 상처를 — 세 번 새기시오. 한 번에 깊이 가르는 자에게만 보이는 자리가 있소.",
    activeText: (have, need) =>
      `한 번에 2,000 의 일격은 — 자네 날이 충분히 깊어야 닿소. ${have}/${need}`,
    doneText:
      "별빛을 가른 순간을 — 별빛이 세 번 기억했소. 별빛 한 줄기 — 자네에게 새겨 두었소.",
  ...
  },
  {
    ...
    activeText: (have, need) =>
      `한 점도 흔들리지 않고 별빛 앞에 서는 일은 — 매번이 새 시작이오. ${have}/${need}`,
    doneText:
      "다섯 번을 흔들림 없이 마쳤소. 흔들리지 않는 자세 — 자네의 깊이오.",
  ...
  // 천공인의 왕 (Lv80, 폐도 게이트 의뢰 완수 후)
  {
    ...
    offerText:
      "천공인의 왕 협동 토벌에서 GOLD 이상 보상에 한 번이라도 닿아 보시오. 폐도가 자네를 알아보는 첫 표식이오.",
    activeText: (have, need) =>
  ...
  },
  {
    ...
    offerText:
      "천공인의 왕에게 단 한 번의 일격으로 3,000 의 상처를 — 세 번 새기시오. 폐도가 한 자루 칼에도 흔들리는 순간이 있소.",
    activeText: (have, need) =>
      `한 번에 3,000 의 일격은 — 날이 비로소 깊어진 자만 닿소. ${have}/${need}`,
    doneText:
      "폐도의 왕이 자네 일격에 세 번 흔들렸소. 폐도의 일격 — 자네에게 새겨 두었소.",
    acceptLabel: "폐도의 일격 되겠다고 한다",
  },
  {
    id: "star-haven-king-challenge-survive",
    offerText:
      "천공인의 왕 앞에서 다섯 번을 — 단 한 번도 쓰러지지 않고 마치시오. 폐도는 견디는 자만이 풀어낼 수 있소.",
    activeText: (have, need) =>
      `폐도 앞에서 흔들리지 않는 일은 — 한 호흡도 어긋나면 안 되오. ${have}/${need}`,
    doneText:
      "폐도의 왕 앞에서 다섯 번을 흔들림 없이 마쳤소. 폐도를 견디는 자 — 자네의 깊이오.",
  ...
  // 창공의 주재 (Lv90, 옥좌 게이트 의뢰 완수 후)
  {
    ...
    offerText:
      "창공의 주재 협동 토벌에서 GOLD 이상 보상에 한 번이라도 닿아 보시오. 옥좌가 자네를 처음으로 깊이 인정하는 표식이오.",
    activeText: (have, need) =>
      `옥좌의 무게는 별빛 끝의 무게요. 한 점이라도 깊이 새기시오. ${have}/${need}`,
    doneText:
  ...
  },
  {
    ...
    activeText: (have, need) =>
      `한 번에 4,500 의 일격은 — 끝에 닿은 자만이 닿소. ${have}/${need}`,
    doneText:
      "옥좌가 자네 일격에 세 번 흔들렸소. 옥좌의 일격 — 자네에게 새겨 두었소.",
    acceptLabel: "옥좌의 일격 되겠다고 한다",
  },
  {
    id: "star-haven-arbiter-challenge-survive",
    offerText:
      "창공의 주재 앞에서 다섯 번을 — 단 한 번도 쓰러지지 않고 마치시오. 옥좌를 견디는 자만이 별빛 끝을 보오.",
    activeText: (have, need) =>
      `옥좌 앞에서 흔들리지 않는 일은 — 끝에 닿은 자만이 할 수 있소. ${have}/${need}`,
    doneText:
      "옥좌 앞에서 다섯 번을 흔들림 없이 마쳤소. 옥좌를 견디는 자 — 별빛 끝까지 본 자요.",
