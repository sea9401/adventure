...
// 그림자 베기 — ATK × 1.5. AP cost 3. (2026-05-23 DEF 무시 제거 — DEF 적용)
const SHADOW_CUT = getAPSkillByName("그림자 베기")!;

// damageBetween(10, 3) = 7. AP fired: ATK ×1.5 = 15 → damageBetween(15, 3) = 12.
...
describe("AP 스킬 시스템 — 그림자 베기", () => {
  ...
  it("AP 장착 시 시작 AP=3, 첫 턴에 발동", () => {
    ...
    // 발동 시점에 state.ap = 3 ≥ 3 → 발동. ATK 15 DEF 3 → damageBetween(15,3)=12. AP 3+1-3=1.
    expect(s.enemyHp).toBe(88);
    expect(s.ap).toBe(1);
  });

  it("AP 3 시작 턴 — ATK ×1.5 (DEF 적용, damage 12)", () => {
    const p: PlayerCombat = { ...PLAYER, equippedAPSkills: [eq(SHADOW_CUT)] };
    let s = initialBattleState(p, enemy(1000), "용사");
    s = advanceTurn(s, p, "용사"); // turn 1: 발동, ATK 15 DEF 3 → 12 데미지, AP 3+1-3=1
    expect(s.enemyHp).toBe(1000 - 12);
    expect(s.ap).toBe(1);
  });
  ...
  it("AP 스킬은 첫 공격에만 발동 — 같은 턴 추가타엔 평타", () => {
    ...
    let s = initialBattleState(p, enemy(1000), "용사");
    // turn 1: 1st AP 3≥3 → 발동 (12 데미지), 2nd 는 평타 7. AP 3+1-3+1 = 2.
    s = advanceTurn(s, p, "용사"); // 1st
    expect(s.enemyHp).toBe(988);
    s = advanceTurn(s, p, "용사"); // 2nd, attacksLeft 0 → 턴 종료
    expect(s.enemyHp).toBe(981);
    expect(s.ap).toBe(2);
    ...
    expect(s.enemyHp).toBe(981 - 7);
    s = advanceTurn(s, p, "용사"); // 2nd of turn 2 — 평타
    expect(s.enemyHp).toBe(981 - 7 - 7);
  });
...
describe("AP 스킬 — 천살 (ignoresEvasion) — 회피 100% 우회", () => {
  it("회피 100% 적 상대로도 큰 한 방 — ATK ×3 (회피 무시)", () => {
    ...
    // (2026-05-23 DEF 무시 제거 — DEF 격리 위해 def 0. 검증 대상은 회피 우회.)
    ...
    // ATK 10 × 3.0 = 30 (DEF 0). damageBetween variance → 25 이상.
...
describe("AP 스킬 멀티 발동", () => {
  ...
  it("공격형 2개 슬롯은 한 턴에 첫 공격형 1개만 발동", () => {
    ...
    let s = initialBattleState(p, enemy(1000), "용사");
    s = advanceTurn(s, p, "용사");
    // 천살 ATK 10×3=30, DEF 3 적용 → damageBetween(30,3)=27. (2026-05-23 DEF 무시 제거)
    expect(s.enemyHp).toBe(973);
    expect(s.ap).toBe(3);
