...
describe("PvP AP — 초기화", () => {
  ...
  it("p1 만 장착 → p1.ap = 시작 AP(3), p2.ap = 0", () => {
...
describe("PvP AP — 그림자 베기 (atk_multiplier + ignoresDef)", () => {
  ...
  it("p1 발동 턴 — 평타보다 큰 한 방", () => {
    ...
    expect(dmgTurn1).toBeGreaterThan(dmgTurn2);
    // turn1 발동 후 AP 3+1-3=1, turn2 미발동 후 +1=2.
  ...
  it("p2 도 동등하게 발동 — 양면 대칭", () => {
    ...
    s = attack(s);
    expect(dmgP2Turn1).toBeGreaterThan(beforeP2Turn2 - s.p1.hp);
    expect(s.p2.ap).toBe(2);
  });
});
...
describe("PvP AP — 천살 (ignoresEvasion) — 회피 100% 우회", () => {
  it("회피 100% 적 상대로도 큰 한 방 발동 — dodge cascade 우회", () => {
    ...
    // 2026-05-23 DEF 무시 제거 — DEF 격리(def 0). 검증 대상은 회피(dodge cascade) 우회.
...
describe("PvP AP — 턴당 1회 정책 + AP 회복", () => {
  it("같은 턴 추가타엔 AP 스킬 재발동 안 함", () => {
    ...
    let s = initialBattleStatePvP(p1, BASE, "p1", "p2");
    // 첫 턴 (p1): 첫 공격 AP 3 → 발동, 둘째 공격 평타. AP 3+1-3+1 = 2.
    ...
    expect(s.p1.turn.apSkillFiredThisTurn).toBeNull(); // 페이즈 종료 시 turn 상태 리셋.
    expect(s.p1.ap).toBe(2); // 3 → 1 → 2
  });
  ...
  it("회피 당해도 attacker AP +1 (행동 시도이므로)", () => {
    ...
    expect(s.p1.ap).toBe(4); // +1 (cost 차감 X — 발동 안 됨)
    expect(s.p1.turn.apSkillFiredThisTurn).toBeNull();
  });
});
...
describe("PvP AP — 광기 (player_atk_buff + def_debuff)", () => {
  it("발동 시 ATK 버프 + DEF 디버프 셋업", () => {
    ...
    let s = initialBattleStatePvP(p1, BASE, "p1", "p2");
    s = attack(s); // p1 turn 1: AP 3 → 광기 발동
  ...
  it("발동턴 그림자 분신 추가타에도 광기 ATK 보너스 적용 (회귀 가드)", () => {
    ...
    s = attack(s); // p1 turn 1: 광기 발동, 메인 130 + 분신 130 = 260.
    expect(s.p1.buffs.playerAtkBuffTurnsLeft).toBe(3);
...
describe("PvP AP — 둔화·폭주 (SPD mult)", () => {
  ...
  it("폭주 — 발동 후 자기 SPD ×1.5", () => {
    ...
    let s = initialBattleStatePvP(p1, BASE, "p1", "p2");
    s = attack(s); // turn 1: AP 3 < 4
    s = attack(s); // p2
    s = attack(s); // turn 2: AP 4 → 폭주 발동
...
describe("PvP AP — 집중의 호흡 (crit_buff_next_attack)", () => {
  it("발동 후 다음 평타가 강제 크리 + 크리뎀 보너스", () => {
    ...
    let s = initialBattleStatePvP(p1, BASE, "p1", "p2");
    s = attack(s); // p1 첫 공격 — AP 3 → 집중호흡 발동, focusedBreathCritDmgBonusPct 셋
...
describe("PvP AP — 연환격 (extra_attack_this_turn)", () => {
  it("발동 시 이번 턴 attacksLeft +1", () => {
    ...
    let s = initialBattleStatePvP(p1, BASE, "p1", "p2");
    s = attack(s); // turn 1: AP 3 → 연환격 발동, attacksLeft +1
...
describe("PvP AP — 폭풍 일격 (atk_plus_spd_pct_bonus)", () => {
  it("발동 시 본타 + (ATK × spdPct/100) 추가 데미지", () => {
    ...
    let s = initialBattleStatePvP(p1, BASE, "p1", "p2");
    s = attack(s); // turn 1: AP 3 → 발동
    const dmgStorm = BASE.hp - s.p2.hp;
    s = attack(s); // p2
    const before = s.p2.hp;
    s = attack(s); // turn 2: AP 1 < 3 → 평타
    const dmgPlain = before - s.p2.hp;
...
describe("PvP AP — 광살참 (multi_hit_self_damage)", () => {
  it("발동 시 멀티히트 + 자해 HP", () => {
    ...
    let s = initialBattleStatePvP(p1, BASE, "p1", "p2");
    // AP 4 까지 충전 필요.
    s = attack(s); // turn 1 (p1 AP 3 → 4 후 phase 종료) AP=4
    s = attack(s); // p2
    const beforeP2Hp = s.p2.hp;
    const beforeP1Hp = s.p1.hp;
    s = attack(s); // turn 2 p1 AP 4 → 광살참 발동
...
describe("PvP AP — 정화 (cleanse_debuffs)", () => {
  it("발동 시 AP 1 cost 소비 (cleanse 자체는 idempotent)", () => {
    ...
    let s = initialBattleStatePvP(p1, BASE, "p1", "p2");
    // 첫 턴 AP 3, PURIFY cost 1 → 발동. 회복 +1 후 -1 = 3. 그대로.
    s = attack(s);
    expect(s.p1.ap).toBe(3);
  });
});
...
describe("PvP AP — 잔상 (block_next_enemy_attack)", () => {
  it("발동 후 상대 다음 공격 1회 무효 + 카운터 1 소비", () => {
    ...
    let s = initialBattleStatePvP(p1, BASE, "p1", "p2");
    // AP 3 시작이라 첫 턴 발동.
    s = attack(s); // p1 turn 1 AP 3 → 잔상 발동
...
describe("PvP AP — 슬롯 발동 조건", () => {
  it("ap_at_least 조건 — AP 임계 미달 시 발동 안 함", () => {
    ...
    let s = initialBattleStatePvP(p1, BASE, "p1", "p2");
    // 첫 턴 AP 3 — cost 3 충족 but 조건 (AP ≥ 5) 불충족.
    s = attack(s);
    expect(s.p1.turn.apSkillFiredThisTurn).toBeNull();
    expect(s.p1.ap).toBe(4); // 발동 안 했으므로 cost 차감 X
  });
...
describe("PvP AP — 멀티 발동", () => {
  ...
  it("p1 공격형 2개 슬롯은 첫 공격형 1개만 발동", () => {
    ...
    let s = initialBattleStatePvP(p1, BASE, "p1", "p2");
    s = attack(s);
    // 천살 ATK 10×3=30, DEF 5 적용 → damageBetween(30,5)=25. (2026-05-23 DEF 무시 제거)
    expect(BASE.hp - s.p2.hp).toBe(25);
    expect(s.p1.ap).toBe(3);
