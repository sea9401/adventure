...
describe("별을 잊은 것 — 별빛 고리 드랍 (gold 부터, 티어 오를수록 상승)", () => {
  it("ringRoll 이 도달 티어 값으로 덮어써져 resolve 까지 전달된다", () => {
    ...
    expect(computeCoopReward("별을 잊은 것", "gold").ringRoll?.chance).toBe(0.05);
    expect(computeCoopReward("별을 잊은 것", "epic").ringRoll?.chance).toBe(0.1);
    expect(computeCoopReward("별을 잊은 것", "legend").ringRoll?.chance).toBe(0.15);
  });
